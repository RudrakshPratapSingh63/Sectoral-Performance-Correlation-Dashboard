...
#             if c2.button("➖", key=f"rm_{i}") and len(st.session_state.tickers) > 1:
#                 st.session_state.tickers.pop(i); st.rerun()
#         if st.button("➕ Add Sector", use_container_width=True): st.session_state.tickers.append(names[0]); st.rerun()
...
# 
#     # TABS
#     t1, t2, t3, t4 = st.tabs(["📈 MARKET", "📉 RISK LAB", "⚖️ PORTFOLIO", "📄 REPORT"])
# 
#     with t1:
...
#         if st.button("🖨️ GENERATE ANALYST REPORT PDF"):
